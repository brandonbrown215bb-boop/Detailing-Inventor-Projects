...
async function applySavedProject(parsed, { filePath = '', quiet = false } = {}) {
  ...
  state.loadErrors = [];
  state.scanSource = surfaces.length ? parsed.scanSource || 'saved' : 'none';
...
async function importBomSpreadsheet() {
  ...
  try {
    ...
    let message = `Imported BOM (${bom.plan?.stats?.folderCount || 0} shell folders planned).`;
...
function renderDetailChecklist(record) {
...
async function pickShellRoot({ relocate = false } = {}) {
  ...
  setAppStatus(relocate ? `Shell root relocated to ${result.folderPath}` : `Shell root set to ${result.folderPath}`);
  setTimeout(() => setAppStatus(''), 5000);
}
...
async function ensureShellRootAvailable() {
  ...
  const retry = confirm(
    `The shell root folder was not found:\n${bom.shellRoot}\n\nChoose a new location?`
  );
...
async function createShellFolders() {
  ...
  if (!rootPath) return;
  const relativePaths = bom.plan.entries.map((entry) => entry.relativePath);
  try {
    const result = await api.createShellFolders(rootPath, relativePaths);
    const next = attachShellRoot({
      ...bom,
      foldersCreatedAt: new Date().toISOString(),
    }, rootPath);
    setBomState(next);
    setAppStatus(`Created ${result.count} shell folder${result.count === 1 ? '' : 's'} under ${rootPath}`);
    setTimeout(() => setAppStatus(''), 6000);
  } catch (err) {
...
async function openBomAssemblyFolder(entry) {
  ...
  try {
    await api.openShellFolder(rootPath, entry.relativePath);
  } catch (err) {
    const retry = confirm(`${err.message || err}\n\nChoose a new shell root?`);
    if (retry) {
      await pickShellRoot({ relocate: true });
      const nextRoot = getBomState().shellRoot;
      if (nextRoot) {
        try {
          await api.openShellFolder(nextRoot, entry.relativePath);
        } catch (err2) {
