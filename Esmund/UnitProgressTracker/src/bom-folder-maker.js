/** BOM 391- shell folder planning for Unit Progress Tracker. */
...
export function resolveSegmentFolder(skid, segment) {
  ...
  const hit = order.find((entry) => entry.normalized === normalized);
  return hit ? hit.folderPrefix : null;
}
...
export function buildShellFolderPlan(rows, { shellRoot = null } = {}) {
  ...
  for (const row of p391) {
    ...
    const segmentPath = `Shell/Skid ${skidNum}/${segmentFolder}`;
