...
const { walk391ZIamFiles, readConfigsFromIams, extractSkidHintFromPath, cancelActiveScan, beginScanSession, resolveIamPath, iamPathKey, iamBasenameKey } = require('./src/iam-scan');
...
function resolveScanInputs(inputs) {
  const iamPaths = [];
  ...
  for (const raw of inputs || []) {
    ...
    if (!trimmed) continue;
    if (/\.iam$/i.test(trimmed)) {
      if (fs.existsSync(trimmed)) {
        iamPaths.push(resolveIamPath(trimmed));
        folderRoots.add(path.dirname(trimmed));
      }
      continue;
    }
    if (!fs.existsSync(trimmed)) continue;
    folderRoots.add(trimmed);
    iamPaths.push(...walk391ZIamFiles(trimmed));
  }

  const seen = new Set();
  const uniquePaths = [];
  for (const iamPath of iamPaths) {
    const resolved = resolveIamPath(iamPath);
    const key = iamPathKey(resolved);
  ...
  return {
    iamPaths: uniquePaths,
    folderPath: folderRoots.size === 1 ? [...folderRoots][0] : null,
  };
}

function buildSurfaceFromConfig(configJson, filePath, folderPath, sourceType) {
...
async function scanIamPathList(iamPaths, folderPath, onProgress) {
  if (!iamPaths.length) {
    return { surfaces: [], errors: [], scanSource: 'none', iamDiscovered: 0 };
  }

  const resolvedPaths = iamPaths.map((p) => resolveIamPath(p));
  const rootFolder = folderPath || path.dirname(resolvedPaths[0]);

  onProgress?.({
    phase: 'discovered',
    total: resolvedPaths.length,
    message: `Found ${resolvedPaths.length} 391Z assembl${resolvedPaths.length === 1 ? 'y' : 'ies'} — starting Inventor read…`,
  });
  ...
  let readResult;
  try {
    readResult = await readConfigsFromIams(__dirname, resolvedPaths, onProgress);
  } catch (e) {
    return {
      surfaces: [],
      errors: resolvedPaths.map((filePath) => ({ filePath, error: e.message || String(e) })),
      scanSource: 'iam',
      iamDiscovered: resolvedPaths.length,
    };
  }

  const configByPath = new Map();
  const configByBasename = new Map();
  for (const entry of readResult.surfaces || []) {
    if (!entry?.config) continue;
    configByPath.set(iamPathKey(entry.iamPath), entry.config);
    configByBasename.set(iamBasenameKey(entry.iamPath), entry.config);
  }

  for (const err of readResult.errors || []) {
    const filePath = err.iamPath || err.filePath;
    if (!filePath) continue;
    errors.push({ filePath: resolveIamPath(filePath), error: err.error || 'Inventor read failed' });
  }

  onProgress?.({ phase: 'building', current: 0, total: resolvedPaths.length, message: 'Building surface models…' });

  for (let i = 0; i < resolvedPaths.length; i++) {
    const filePath = resolvedPaths[i];
    const key = iamPathKey(filePath);
    const baseKey = iamBasenameKey(filePath);
    onProgress?.({
      phase: 'building',
      current: i + 1,
      total: resolvedPaths.length,
    ...
    const configJson = configByPath.get(key) || configByBasename.get(baseKey);
    if (!configJson) {
      const inventorErr = errors.find(
        (e) => iamPathKey(e.filePath) === key || iamBasenameKey(e.filePath) === baseKey
      );
      if (!inventorErr) {
        const got = (readResult.surfaces || []).length;
        const failed = (readResult.errors || []).length;
        errors.push({
          filePath,
          error:
            got || failed
              ? `Inventor read finished (${got} ok, ${failed} failed) but this IAM was not in the results`
              : 'Inventor read returned no config for this IAM',
        });
      }
      continue;
    }
    ...
    const surface = buildSurfaceFromConfig(configJson, filePath, rootFolder, 'iam');
    if (!surface) {
      errors.push({ filePath, error: 'Config found but no geometry boxes (wall/roof/unitBase)' });
      continue;
    }
  ...
  return {
    ...
    errors,
    scanSource: 'iam',
    readSource: readResult.source || 'inventor',
    iamDiscovered: resolvedPaths.length,
  };
}
...
async function scanIamSurfaces(folderPath, onProgress) {
  ...
  return scanIamPathList(iamPaths, folderPath, onProgress);
}
...
ipcMain.handle('pick-folders', async (event) => {
  const result = await dialog.showOpenDialog(parentWindowFromEvent(event), {
    title: 'Choose folder(s) containing 391Z surface assemblies (.iam)',
...
async function scanFoldersForSurfaces(folderPaths, onProgress, { iamPaths: explicitIamPaths = null } = {}) {
  beginScanSession();
  const allSurfaces = [];
  const allErrors = [];
  const seenNumbers = new Set();
  ...
  const inputs = Array.isArray(folderPaths) ? [...folderPaths] : [];
  if (Array.isArray(explicitIamPaths) && explicitIamPaths.length) {
    for (const iam of explicitIamPaths) inputs.push(iam);
  }

  const resolved = resolveScanInputs(inputs);
  iamDiscovered = resolved.iamPaths.length;

  if (resolved.iamPaths.length) {
    onProgress?.({
      phase: 'discovering',
      message: `Found ${resolved.iamPaths.length} 391Z IAM file(s)…`,
    });
    let scanResult = await scanIamPathList(resolved.iamPaths, resolved.folderPath, onProgress);
    if (scanResult.scanSource !== 'none') scanSource = scanResult.scanSource;
    iamDiscovered = scanResult.iamDiscovered ?? iamDiscovered;

    for (const surface of scanResult.surfaces) {
      if (seenNumbers.has(surface.surfaceNumber)) {
        allErrors.push({
          filePath: surface.filePath,
          error: `Duplicate surface number ${surface.surfaceNumber}`,
        });
        continue;
      }
      seenNumbers.add(surface.surfaceNumber);
  ...
  for (let fi = 0; fi < (folderPaths || []).length; fi++) {
    ...
    if (/\.iam$/i.test(folderPath.trim())) continue;
    if (!fs.existsSync(folderPath)) {
      allErrors.push({ filePath: folderPath, error: 'Folder not found' });
      continue;
    }

    if (resolved.iamPaths.length) continue;

    onProgress?.({
      phase: 'discovering',
      message:
        folderPaths.length > 1
          ? `Scanning folder ${fi + 1} of ${folderPaths.length}…`
          : 'Searching folder for 391Z assemblies…',
    });

    let scanResult = await scanIamSurfaces(folderPath, onProgress);
    if (scanResult.scanSource !== 'none') scanSource = scanResult.scanSource;
    iamDiscovered += scanResult.iamDiscovered ?? 0;
  ...
  allSurfaces.sort((a, b) => a.surfaceNumber.localeCompare(b.surfaceNumber, undefined, { numeric: true }));

  return {
    surfaces: allSurfaces,
    errors: allErrors,
    scanSource: scanSource || 'none',
    iamDiscovered,
  };
}

ipcMain.handle('scan-surfaces-in-folders', async (event, { folderPaths, iamPaths }) => {
  if (!folderScansAllowed) {
    throw new Error('Folder scan blocked during startup. Close and reopen the app, then try again.');
  }
  const folders = Array.isArray(folderPaths) ? folderPaths.filter(Boolean) : [];
  const iams = Array.isArray(iamPaths) ? iamPaths.filter(Boolean) : [];
  if (!folders.length && !iams.length) throw new Error('No folder or IAM file selected');

  const onProgress = (payload) => sendScanProgress(event, payload);
  return scanFoldersForSurfaces(folders, onProgress, { iamPaths: iams });
});

ipcMain.handle('pick-surface-iam', async (event) => {
  const result = await dialog.showOpenDialog(parentWindowFromEvent(event), {
    title: 'Choose 391Z surface assembly (.iam)',
    properties: ['openFile'],
    filters: [{ name: 'Inventor assembly', extensions: ['iam'] }],
  });
  if (result.canceled || !result.filePaths.length) return null;
  return result.filePaths[0];
});

ipcMain.handle('pick-surface-iams', async (event) => {
  const result = await dialog.showOpenDialog(parentWindowFromEvent(event), {
    title: 'Choose 391Z surface assembly file(s)',
...
ipcMain.handle('write-geometry-cache', async (_event, { folderPath, surfaces, scanSource }) => {
  if (!folderPath) throw new Error('No project folder');
  if (!Array.isArray(surfaces)) throw new Error('Invalid surfaces payload');
  writeGeometryCache(folderPath, surfaces, scanSource || 'iam');
  return { ok: true, count: surfaces.length };
});
...
ipcMain.handle('pick-import-unit-config', async (event) => {
  const result = await dialog.showOpenDialog(parentWindowFromEvent(event), {
    title: 'Import unit Config.xml',
    properties: ['openFile'],
    filters: [{ name: 'Unit Config', extensions: ['xml'] }],
  });
  if (result.canceled || !result.filePaths.length) return { canceled: true };
  const filePath = result.filePaths[0];
  try {
    const text = fs.readFileSync(filePath, 'utf8');
    if (!/<\s*(root:)?AHU\b/i.test(text) && !text.includes('segmentList')) {
      throw new Error('File does not look like a unit Config.xml');
    }
    return { canceled: false, filePath, text };
  } catch (e) {
    throw new Error(`Could not read Config.xml: ${e.message || e}`);
  }
});
...
ipcMain.handle('create-shell-folders', async (_event, { rootPath, relativePaths, folders }) => {
  if (!rootPath || !fs.existsSync(rootPath)) {
    throw new Error('Inventor root folder not found');
  }

  const specs = Array.isArray(folders)
    ? folders
    : (Array.isArray(relativePaths) ? relativePaths : []).map((relativePath) => ({ relativePath }));
  ...
  for (const spec of specs) {
    ...
    if (spec.referenceRelativePath) {
      const refFile = path.join(full, 'MODEL_REFERENCE.txt');
      const text =
        `Model folder for this BOM line (shared part across skids):\r\n` +
        `${spec.referenceRelativePath}\r\n\r\n` +
        `Export the Inventor assembly once to the primary skid folder above.\r\n`;
      fs.writeFileSync(refFile, text, 'utf8');
    }
    created.push(full);
  }
