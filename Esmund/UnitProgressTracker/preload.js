...
contextBridge.exposeInMainWorld('unitSurfaceViewer', {
  ...
  pickSurfaceIam: () => ipcRenderer.invoke('pick-surface-iam'),
  pickSurfaceIams: () => ipcRenderer.invoke('pick-surface-iams'),
  ...
  scanSurfacesInFolders: (folderPaths, onProgress, iamPaths) => {
    const listener = (_event, payload) => {
      if (typeof onProgress === 'function') onProgress(payload);
    };
    ipcRenderer.on('scan-progress', listener);
    return ipcRenderer
      .invoke('scan-surfaces-in-folders', {
        folderPaths: Array.isArray(folderPaths) ? folderPaths : [],
        iamPaths: Array.isArray(iamPaths) ? iamPaths : undefined,
      })
      .finally(() => {
        ipcRenderer.removeListener('scan-progress', listener);
      });
  ...
  writeGeometryCache: (folderPath, surfaces, scanSource) =>
    ipcRenderer.invoke('write-geometry-cache', { folderPath, surfaces, scanSource }),
