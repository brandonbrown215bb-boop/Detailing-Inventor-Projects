...
function formatIncrementalScanFailure(result) {
  ...
  if (iamCount === 0) {
    ...
    lines.push('• Use "py -3" if "python" opens the Microsoft Store (Windows app execution alias)');
    lines.push('• pip install pywin32 — or set USV_PYTHON to full path of python.exe');
  }
...
async function applyFolderLoadResult(folderPath, result, { quiet = false, rescan = false, fromCache = false } = {}) {
  ...
  if (rescan) {
    ...
    const sourceLabel = '391Z assemblies (via Inventor)';
    if (!quiet) {
      setAppStatus(
    ...
    if (fromCache) {
      ...
      setAppStatus(`Loaded ${state.surfaces.length} surfaces from IAM via Inventor.`);
    }
  }
