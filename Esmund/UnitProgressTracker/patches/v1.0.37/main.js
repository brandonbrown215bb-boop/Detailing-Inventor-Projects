...
const { walk391ZIamFiles, readConfigsFromIams, extractSkidHintFromPath, cancelActiveScan, beginScanSession, resolveIamPath, iamPathKey } = require('./src/iam-scan');
...
async function scanIamPathList(iamPaths, folderPath, onProgress) {
  ...
  const configByPath = new Map(
    (readResult.surfaces || []).map((entry) => [iamPathKey(entry.iamPath), entry.config])
  );

  for (const err of readResult.errors || []) {
    const filePath = err.iamPath || err.filePath;
    if (!filePath) continue;
    errors.push({ filePath, error: err.error || 'Inventor read failed' });
  }
  ...
  for (let i = 0; i < resolvedPaths.length; i++) {
    ...
    const configJson = configByPath.get(key);
    if (!configJson) {
      if (!errors.some((e) => iamPathKey(e.filePath) === key)) {
        errors.push({
          filePath,
          error: 'Inventor read completed but DOCUMENT_CONFIG_JSON was not returned for this IAM',
        });
      }
      continue;
    }
