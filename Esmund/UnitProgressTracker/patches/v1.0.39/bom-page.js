...
import {
  normalizeUnitConfig,
  getConfigSegmentsForSkid,
  unitConfigForPersistence,
  segmentPrefixFromBomColumn,
} from './unit-config-parser.js';
import {
  applyBomListDisplay,
  getBomFilterOptions,
  BOM_SORT_OPTIONS,
...
export function bomForPersistence(bom) {
  ...
  return {
    ...
    unitConfig: unitConfigForPersistence(bom.unitConfig),
    entryRecords:
...
export function buildBomPlan(bom) {
  ...
  const unitConfig = normalizeUnitConfig(bom?.unitConfig);
  const plan = buildShellFolderPlan(rows, {
    shellRoot: bom?.shellRoot || null,
    unitConfig,
  });
  ...
  plan.stats.configLoaded = Boolean(unitConfig);
  plan.configWarnings = unitConfig?.warnings || [];
...
export function normalizeBomState(raw) {
  ...
  const bom = {
    ...
    unitConfig: normalizeUnitConfig(base.unitConfig),
...
export function bomStateFromImport(filePath, rows, prior = null) {
  ...
  return normalizeBomState({
    ...
    unitConfig: prior?.unitConfig || null,
    entryRecords: prior?.entryRecords || {},
  });
}

export function attachUnitConfig(bom, unitConfig) {
  return normalizeBomState({ ...bom, unitConfig });
}
...
export function addManualBomEntry(bom, fields) {
  ...
  if (!created) {
    const hint = bom?.unitConfig
      ? 'Entry could not be placed — check skid number and segment code match the unit Config.'
      : 'Entry could not be placed — import unit Config.xml or check skid bracket matches segment.';
...
/** Segment choices for the add-entry dialog — Config shipping skids when loaded, else BOM/bracket. */
export function getSegmentOptionsForSkid(bom, skid) {
  const skidTrim = String(skid || '').trim();
  if (!skidTrim) return [];

  const skidNum = parseSkidNumber(skidTrim);
  const configSegments = skidNum ? getConfigSegmentsForSkid(bom?.unitConfig, skidNum) : [];

  const byCode = new Map();
  for (const row of allSourceRows(bom)) {
    if (String(row.Skid).trim() !== skidTrim) continue;
    if (!row.Segment || row.Segment === '<--') continue;
    const code = normalizeSegmentCode(segmentPrefixFromBomColumn(row.Segment));
    byCode.set(code, row.Segment);
  }

  if (configSegments.length) {
    return configSegments.map((seg) => {
      const value = byCode.get(seg.normalized) || `${seg.tagName} - ${seg.segmentType || seg.tagName}`;
      const label = byCode.get(seg.normalized)
        ? `${seg.folderPrefix} — ${value}`
        : `${seg.folderPrefix} — ${seg.tagName}`;
      return { value, label };
    });
  }
...
export function renderBomPage(container, bom, handlers, listDisplay = null, viewOptions = null) {
  ...
  configBtn.textContent = bom?.unitConfig ? 'Replace Config…' : 'Import Config…';
  configBtn.addEventListener('click', () => handlers.onImportConfig?.());
  ...
  createBtn.textContent = 'Create Inventor folders…';
  ...
  rootBtn.textContent = bom?.shellRoot ? 'Relocate Inventor root…' : 'Set Inventor root…';
  rootBtn.disabled = !hasSource;
  rootBtn.addEventListener('click', () => handlers.onPickShellRoot?.());

  toolbar.append(importBtn, configBtn, addBtn, createBtn, rootBtn);
  ...
  if (bom?.unitConfig) {
    const skidSummary = bom.unitConfig.skids
      .map((sk) => `Skid ${String(sk.id).padStart(2, '0')}: [${escapeHtml(sk.bracket)}]`)
      .join(' · ');
    meta.innerHTML += `<div><span class="bom-meta-label">Config:</span> ${escapeHtml(shortPath(bom.unitConfig.sourceFile))}</div>`;
    meta.innerHTML += `<div class="bom-meta-muted">${skidSummary}</div>`;
  } else if (hasSource) {
    meta.innerHTML += '<div class="bom-hint">Import unit Config.xml so segment folders match shipping skids (recommended).</div>';
  }
  if (bom?.shellRoot) {
    meta.innerHTML += `<div><span class="bom-meta-label">Inventor root:</span> ${escapeHtml(bom.shellRoot)}</div>`;
  } else if (hasSource) {
    meta.innerHTML += `<div class="bom-hint">Pick an Inventor root before creating folders or opening export targets.</div>`;
  }
  ...
  if (!hasSource) {
    meta.innerHTML += '<div class="bom-hint">Import a BOM xlsx or add entries manually for 391- Inventor export folders.</div>';
  }
  ...
  if (plan?.configWarnings?.length) {
    const alert = document.createElement('div');
    alert.className = 'bom-alert';
    alert.textContent = plan.configWarnings.join(' ');
    container.appendChild(alert);
  }

  if (plan?.stats && !plan.stats.configLoaded && hasSource) {
    const hint = document.createElement('div');
    hint.className = 'bom-hint';
    hint.textContent =
      plan.stats.skippedCount > 0
        ? `${plan.stats.skippedCount} BOM row(s) skipped — import Config.xml to resolve segment placement (BOM skid brackets may not match segment codes).`
        : 'Segment folders use BOM skid brackets until you import Config.xml.';
  ...
  if (plan?.stats) {
    ...
    const duplicatePart = plan.stats.duplicateRefCount ? ` · ${plan.stats.duplicateRefCount} ref` : '';
    stats.textContent =
      `${plan.stats.folderCount} export folder${plan.stats.folderCount === 1 ? '' : 's'} · ` +
      `${plan.stats.excludedCount} excluded · ` +
      `${plan.stats.customSqCount} custom (SQ)${duplicatePart}${manualPart}${removedPart}`;
  ...
  for (const entry of filteredEntries) {
    ...
    if (entry.isDuplicateRef) li.classList.add('bom-row-ref');
    ...
    if (entry.isDuplicateRef) {
      const tag = document.createElement('span');
      tag.className = 'bom-tag-ref';
      tag.textContent = `Ref → Skid ${parseInt(parseSkidNumber(entry.modelSkid) || '0', 10) || '?'}`;
      head.appendChild(tag);
    }
    if (entry.isModelPrimary) {
      const tag = document.createElement('span');
      tag.className = 'bom-tag-primary';
    ...
    openBtn.title = bom.shellRoot ? 'Open this assembly folder in File Explorer' : 'Set Inventor root first';
