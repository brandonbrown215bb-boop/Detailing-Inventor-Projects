...
async function runIncrementalScan(folderPaths) {
  requireApi();
  setScanUiBusy(true);
  try {
    setScanProgress(true, { phase: 'discovering', message: 'Scanning folder…' });
...
async function replaceSurfaceFromFolder() {
  ...
  try {
    const result = await runIncrementalScan([folderPath]);
    if (!result?.surfaces?.length) {
      alert('No 391Z surface geometry found in that folder.');
      return;
    }
...
async function addSurfacesFromFolders() {
  ...
  try {
    ...
    if (!result?.surfaces?.length) {
      alert('No 391Z surface geometry found in the selected folder(s).');
      return;
    }
