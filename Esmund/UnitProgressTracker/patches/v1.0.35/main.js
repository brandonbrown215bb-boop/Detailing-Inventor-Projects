...
async function scanFoldersForSurfaces(folderPaths, onProgress) {
  beginScanSession();
  const allSurfaces = [];
  const allErrors = [];
  const seenNumbers = new Set();
  ...
  return { surfaces: allSurfaces, errors: allErrors, scanSource: scanSource || 'none' };
}

ipcMain.handle('scan-surfaces-in-folders', async (event, { folderPaths }) => {
  if (!folderScansAllowed) {
    throw new Error('Folder scan blocked during startup. Close and reopen the app, then try again.');
  }
  const paths = Array.isArray(folderPaths) ? folderPaths.filter(Boolean) : [];
  if (!paths.length) throw new Error('No folder selected');

  const onProgress = (payload) => sendScanProgress(event, payload);
  return scanFoldersForSurfaces(paths, onProgress);
});
