...
/** Sidecar config when IAM lacks embedded DOCUMENT_CONFIG_JSON (Pigeon export, manual dump, etc.). */
function findSidecarConfigJson(iamPath) {
  const dir = path.dirname(iamPath);
  const base = path.basename(iamPath, path.extname(iamPath));
  const candidates = [
    path.join(dir, `${base}.json`),
    path.join(dir, `${base} DOCUMENT_CONFIG_JSON.txt`),
    path.join(dir, `${base} DOCUMENT_CONFIG_JSON.json`),
    path.join(dir, 'CONFIG_JSON', `${base}.json`),
    path.join(dir, '..', 'CONFIG_JSON', `${base}.json`),
  ];
  for (const candidate of candidates) {
    ...
    try {
      ...
      if (data?.configuration) return { config: data, configPath: candidate };
    } catch {
      /* try next */
    }
  }
...
function resolveScanInputs(inputs) {
  ...
  for (const raw of inputs || []) {
    ...
    if (/\.iam$/i.test(trimmed)) {
      if (fs.existsSync(trimmed)) {
        iamPaths.push(path.normalize(trimmed));
  ...
  const unique = [...new Set(iamPaths.map((p) => path.normalize(p)))];
...
async function scanIamPathList(iamPaths, folderPath, onProgress) {
  ...
  const rootFolder = folderPath || path.dirname(iamPaths[0]);

  onProgress?.({
    phase: 'discovered',
    total: iamPaths.length,
    message: `Found ${iamPaths.length} 391Z assembl${iamPaths.length === 1 ? 'y' : 'ies'} — starting Inventor read…`,
  });
  ...
  for (let i = 0; i < iamPaths.length; i++) {
    ...
    let configJson = configByPath.get(normPath);
    let configPath = null;
    if (!configJson) {
      const sidecar = findSidecarConfigJson(filePath);
      if (sidecar) {
        configJson = sidecar.config;
        configPath = sidecar.configPath;
      }
    }

    if (!configJson) {
      if (!errors.some((e) => path.normalize(e.filePath) === normPath)) {
        errors.push({
          filePath,
          error:
            'No DOCUMENT_CONFIG_JSON on IAM and no sidecar .json found (expected same-name .json or CONFIG_JSON folder)',
        });
      }
      continue;
    }
    ...
    const surface = buildSurfaceFromConfig(configJson, filePath, rootFolder, 'iam', configPath);
    if (!surface) {
...
async function scanFoldersForSurfaces(folderPaths, onProgress, { iamPaths: explicitIamPaths = null } = {}) {
  ...
  if (allSurfaces.length === 0 && folderPaths?.length) {
    for (let fi = 0; fi < folderPaths.length; fi++) {
      const folderPath = folderPaths[fi];
      if (!folderPath || /\.iam$/i.test(String(folderPath).trim()) || !fs.existsSync(folderPath)) continue;
      onProgress?.({ phase: 'discovering', message: 'Searching for CONFIG_JSON files…' });
      const jsonResult = scanJsonSurfaces(folderPath, onProgress);
      if (jsonResult.scanSource !== 'none') scanSource = jsonResult.scanSource;
      for (const surface of jsonResult.surfaces) {
        if (seenNumbers.has(surface.surfaceNumber)) continue;
        seenNumbers.add(surface.surfaceNumber);
        allSurfaces.push(surface);
      }
